...
exports.signup = (req, res, next) => {    
    ...
    if (!regexEmail.test(email)) {
         return res.status(406).json({ error: 'Email erroné !' }) 
    }
    if (!regexPassword.test(password)) {
         return res.status(406).json({ error: 'Mot de passe trop faible !' })  
    }
         ...
         .then(userFound => {
              if (!userFound) { 
                   bcrypt.hash(password, 10) 
                        ...
                        .catch(error => res.status(500).json({ error: "aaa" })) 

              } else {   
                   return res.status(406).json({ error: 'Email ou username déjà pris !' }) 
              }
         })
         .catch(error => res.status(500).json({ error }))
}
         ...
         .then(user => {
              if (user) { 
                   bcrypt.compare(password, user.password)
                        .then(valid => {
                             if (!valid) { 
                                  return res.status(406).json({ error: 'Email ou mot de passe incorrect!' }) 
                             }
                             res.status(200).json({
                                  userId: user.id,
                                  isAdmin: user.isAdmin,
                        ...
                        .catch(error => res.status(403).json({ error: 'Email ou mot de passe incorrect!' })) 
              } else {
              ...
              models.User.findOne({ 
                   ...
                   .then(userAdmin => {
                        if (userAdmin.role != true) { 
                             return res.status(406).json({ error: 'Un problème est survenue lors de la supprésion de l\'utilisateur.' })
                        }
                        user.destroy() 
                        .then(() => res.status(200).json({ message: 'Utilisateur supprimé !' }))
                        .catch(error => { res.status(400).json({ error: 'Impossible de supprimer !' }) }); 
         ...
         .then(user => {
              ...
              if (user.id == userId) {
                   return user.update({ username: username}) 
                        ...
                        .catch(error => res.status(400).json({ error: 'Une erreur est survenue lors de la modification' }));
              }
              models.User.findOne({
                   attributes: ['id', 'role'],
                   where: { id: userId } 
